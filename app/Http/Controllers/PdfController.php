...
class PdfController extends Controller
{
    ...
    public function download(Request $request)
    {
        ...
        if ($typesurat == 'individu') {
            ...
            //atas dasar
            // Set X position for the label column
            $this->fpdf->SetX(25);
            // Label column
            $this->fpdf->Cell(0, 10, "Atas Dasar", 0, 0, 'L');
            $this->fpdf->SetX(75);

            // Set width and height for the cell
            $cellWidth = 100;
            $cellHeight = 6;

            // Content for the value column
            $content = ": $dasar";

            // Output the content with specified width and height
            $this->fpdf->MultiCell($cellWidth, $cellHeight, $content, 0, 'L');
            ...
            $this->fpdf->SetX(25);
            // Label column
            $this->fpdf->Cell(0, 10, "Dalam Rangka", 0, 0, 'L');
            $this->fpdf->SetX(75);
            ...
            $this->fpdf->SetX(0);
            $this->fpdf->MultiCell(175, 0.5, ($jbtvip), 0, 'R', false);
            $this->fpdf->Ln(14); // Menambah spasi 5 unit setelah MultiCell pertama
            $this->fpdf->SetX(0);
            $this->fpdf->MultiCell(175, 0.5, ($namavip), 0, 'R', false);
            $this->fpdf->Ln(4); // Menambah spasi 5 unit setelah MultiCell kedua
            $this->fpdf->SetX(0);
            ...
            $nipbaru = explode(",", $nip);
            $jbtbaru = explode(",", $jabatan);

            foreach ($namabaru as $index => $value) {
                $this->fpdf->Ln();
                $this->fpdf->Cell($columnWidth, 10, $namabaru[$index], 1);
                $this->fpdf->Cell($columnWidth, 10, $nipbaru[$index], 1, 0, 'C');
                $this->fpdf->Cell($columnWidth, 10, $jbtbaru[$index], 1, 0, 'C');
            }
